...
def test_dialog_import_chain_primary_path():
    ui_mod = importlib.import_module("upbit_trader_ui_controller")
...
def test_dialog_import_chain_fallback_path():
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "upbit_dialogs":
            raise ImportError("forced fallback path")
        return real_import(name, globals, locals, fromlist, level)

    with patch("builtins.__import__", side_effect=fake_import):
        sys.modules.pop("upbit_trader_ui_controller", None)
        fallback_mod = importlib.import_module("upbit_trader_ui_controller")
    ...
    sys.modules.pop("upbit_trader_ui_controller", None)
    importlib.import_module("upbit_trader_ui_controller")
